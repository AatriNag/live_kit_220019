...
async def entrypoint(ctx: JobContext):
    ...
    def on_user_input(event: UserInputTranscribedEvent):
        ...
        if "add" in words and "filler" in words:
            # Find the word between "add" and "filler" word_to_add = words[add_idx + 1].strip("'\"")
            try:
                add_idx = words.index("add")
                if add_idx + 1 < len(words):
                    word_to_add = words[add_idx + 1].strip("'\"")
                    async def add_and_log():
                            await agent.interruption_handler.add_filler(detected_lang, word_to_add)
                            logger.info(f" Added filler: '{word_to_add}' for '{detected_lang}'")
                            stats = agent.interruption_handler.get_statistics()
                            fillers = stats["fillers_by_language"].get(detected_lang, [])
                            logger.info(f" Updated fillers for '{detected_lang}': {fillers}")
                    
                    asyncio.create_task(add_and_log())
                    return  
            except Exception as e:
        ...
        elif ("show" in words or "list" in words) and "filler" in words:
            stats = agent.interruption_handler.get_statistics()
            fillers = stats["fillers_by_language"].get(detected_lang, [])
            logger.info(f" Current fillers for '{detected_lang}': {fillers}")
            return
